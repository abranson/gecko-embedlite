%define embedlite_config sailfishos
...
%description
Gecko browser engine with the EmbedLite embedding API.

%package devel
Requires: %{name} = %{version}-%{release}
Provides: xulrunner-qt5-devel = %{version}-%{release}
Obsoletes: xulrunner-qt5-devel < 154
Conflicts: xulrunner-devel
Summary: Development files for Gecko EmbedLite
...
%description devel
Development files for Gecko EmbedLite.

%package misc
Requires: %{name} = %{version}-%{release}
Provides: xulrunner-qt5-misc = %{version}-%{release}
Obsoletes: xulrunner-qt5-misc < 154
Summary: Additional Gecko EmbedLite files

%description misc
Tests and additional files for Gecko EmbedLite.

# Build output directory.
%define BUILD_DIR "$PWD"/../obj-build-sailfishos
...
echo "%{version}" > "$PWD/config/milestone.txt"
...
ln -s libxul.so.${gecko_abi} ${RPM_BUILD_ROOT}%{mozappdir}/libxul.so
